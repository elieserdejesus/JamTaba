...
{
    //using QObject::sender() to get the source of click event (in this case the source is a QAction instance)
    QString nameOfClickedPreset = dynamic_cast<QAction*>(QObject::sender())->data().toString();

    QMessageBox msgBox;
    msgBox.setText("LoadPresetClicked( " + nameOfClickedPreset + " )\nThat feature is not ready yet ...");
    msgBox.exec();
