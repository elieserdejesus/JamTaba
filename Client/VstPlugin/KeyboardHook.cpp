...
LRESULT CALLBACK globalKeyboardHookProcedure(int nCode, WPARAM wParam, LPARAM lParam){
    ...
    if(nCode == HC_ACTION){
        ...
        if(widgetIsQLineEditInstance){
            ...
            default:
                if(eventType == QKeyEvent::KeyPress){ //handling only KeyPress to avoid problems with special characters like 'é' or 'ã'
                    wchar_t buffer[5];
                    BYTE keyboard_state[256];
                    if(GetKeyboardState(keyboard_state)){
                        //Try to convert the key information to text
                        int result = ToUnicodeEx(lowLevelKeyboardStruct->vkCode, lowLevelKeyboardStruct->scanCode, keyboard_state, buffer, 5, lowLevelKeyboardStruct->flags, GetKeyboardLayout(0));
                        if(result > 0){
                            ev = new QKeyEvent(eventType, lowLevelKeyboardStruct->vkCode, Qt::ShiftModifier, QString::fromWCharArray(buffer));
                        }
                    }
                }
            }
