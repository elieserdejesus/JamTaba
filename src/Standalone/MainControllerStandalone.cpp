...
Midi::MidiMessageBuffer MainControllerStandalone::pullMidiBuffer()
{
    ...
    //grab the midi messages generated by midi controllers
    Midi::MidiMessageBuffer midiBuffer(midiDriver->getBuffer());

    //append midi messages generated by vstplugins
    QList<Midi::MidiMessage> receivedMidiMessages = Vst::Host::getInstance()->getReceivedMidiMessages();
    foreach (const Midi::MidiMessage &vstMidiMessage, receivedMidiMessages) {
        midiBuffer.addMessage(vstMidiMessage);
    }

    Vst::Host::getInstance()->clearReceivedMidiMessages();
    return midiBuffer;
}
