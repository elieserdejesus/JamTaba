...
#include <QDebug>
        ...
        currentBeat(0)
    {
        ...
        progressDisplay->setAccentBeats(QList<int>() << 4);
        ...
        QList<int> accents;
        for (int i = 0; i < progressDisplay->getBeatsPerInterval(); i += 4)
            accents << i;

        progressDisplay->setAccentBeats(accents);
